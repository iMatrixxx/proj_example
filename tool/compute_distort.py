...
LastEditors: aoi
LastEditTime: 2024-04-28 21:36:47
Description: 
...
with open(file_path, 'r') as file:
    ...
    for line in file:
        ...
        # gt_x.append(float(str_split[5]) * 221.77)
        # gt_y.append(float(str_split[6]) * 221.77)
        x = float(str_split[0]) - 2560
        y = float(str_split[1].strip('\n')) - 2048
        
        angle = -0.58 * math.pi / 180.0
    ...
    for i in range(len(gt_x)):
        ...
        x_cor = gt_x[i]*(1+x[0] * r2 + x[1] *r2 * r2 ) + x[2] * 2 * obs_x[i] * obs_y[i] + x[3] *(r2 + 2*np.square(obs_x[i]))
        y_cor = gt_y[i]*(1+x[0] * r2 + x[1] *r2 * r2 ) + x[3] * 2 * obs_x[i] * obs_y[i] + x[2] *(r2 + 2*np.square(obs_y[i]))
        
        print("残差：",gt_x[i] - obs_x[i], obs_x[i] - x_cor)
        print("残差：",gt_y[i] - obs_y[i], obs_y[i] - y_cor)
